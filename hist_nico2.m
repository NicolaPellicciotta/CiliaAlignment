...
% BSD License
% July 2013
function [histw,ehistw,mean_bins] = hist_nico2(vv, ww, nbins,vinterval)
...
%  delta = (maxV(1)-minV(1))/nbins;
   good= ind~=0;
   ind=ind(good);ww=ww(good);
   histw=accumarray(ind,ww,[],@median);
   ehistw=accumarray(ind,ww,[],@std)./sqrt((accumarray(ind,ww,[],@numel)));
