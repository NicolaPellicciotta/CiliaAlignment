...
for ii=1:size(d,1)  %%%% load all coordinates and velocities in 4 arrays (xx,yy,uu,vv)
    ...
    cd(bf_var.data_dir);fs=bf_var.mo.read();
    ...
    for b=1:numel(x);
        ...
        if box_ss(b)>n_good_px; 
            froi= fs(floor(y(b)-bs):floor(y(b)+bs),floor(x(b)-bs):floor(x(b)+bs),:);
            [fp,fq,m_pxx] = average_fft_meanpk(froi,temp_ss,bf_var.mo.FrameRate,[10,35]);
            F(b)=fp;
        else 
            F(b)=nan;
        end
    end  %%%%% this is the end of the load Res
    ...
    figure(5);
%    imshow(ss);hold on;
    imagesc(imresize(IF,32,'method','nearest')); colormap(gray);colorbar();
    hold on; axis image
    ...
    bin_res=32;
    [idr,idth,cc,ecc,n_cc] = corr_frequency_theta_func(x(ind),y(ind),F(ind),bin_res,pi/2);
    ...
    fit_bins= idr(idth==0); fit_cc=cc(idth==0);max_cc=20;%floor(numel(cc(idth==0))/2);
    fit_out_x = fit(fit_bins(1:max_cc),fit_cc(1:max_cc),ft,fo);
    Res.fov(ii).freq.fit_out_x=fit_out_x;

    fit_bins= idr(idth==pi/2); fit_cc=cc(idth==pi/2);max_cc=20;%floor(numel(cc(idth==pi/2))/2);
    fit_out_y = fit(fit_bins(1:max_cc),fit_cc(1:max_cc),ft,fo);
    Res.fov(ii).freq.fit_out_y=fit_out_y;

    figure(4)
     subplot(2,1,1)
     plot(fit_out_x,idr(idth==0),cc(idth==0),'ko');hold on;
     plot(fit_out_y,idr(idth==pi/2),cc(idth==pi/2),'ro');
     legend({'x','y'});
     xlabel('pixel [$\mu$m]','Interpreter','latex');
     ylabel('frequency correlation','Interpreter','latex');
    ...
    bin_res= 32; 
    [idr,idth,cc,ecc,n_cc] = corr_orientation_theta_func(x(ind),y(ind),nu,nv,bin_res,pi/2);
    ...
    fit_bins= idr(idth==0); fit_cc=cc(idth==0);max_cc=floor(numel(cc(idth==0))/2);
    fit_out_x = fit(fit_bins(1:max_cc),fit_cc(1:max_cc),ft,fo);
    Res.fov(ii).fit_out_x=fit_out_x;

    fit_bins= idr(idth==pi/2); fit_cc=cc(idth==pi/2);max_cc=floor(numel(cc(idth==pi/2))/2);
    fit_out_y = fit(fit_bins(1:max_cc),fit_cc(1:max_cc),ft,fo);
    Res.fov(ii).fit_out_y=fit_out_y;
    ...
    FF(start+1:start+numel(x(ind))) = F(ind);  %%%%% load all the frequencies
...
[idr,idth,cc,ecc,n_cc] = corr_orientation_theta_func(XX,YY,nu,nv,bin_res,pi/2);
Res.idr =idr; Res.idth =idth; Res.cc=cc; Res.ecc=ecc; Res.n_cc=n_cc;
    ...
    fit_bins= idr(idth==0); fit_cc=cc(idth==0);max_cc=20;%floor(numel(cc(idth==0))/2);
    fit_out_x = fit(fit_bins(1:max_cc),fit_cc(1:max_cc),ft,fo);
    Res.fit_out_x=fit_out_x;

    fit_bins= idr(idth==pi/2); fit_cc=cc(idth==pi/2);max_cc=30;%floor(numel(cc(idth==pi/2))/2);
    fit_out_y = fit(fit_bins(1:max_cc),fit_cc(1:max_cc),ft,fo);
    Res.fit_out_y=fit_out_y;
...
Res.freq.idr =idr; Res.freq.idth =idth; Res.freq.cc=cc; Res.freq.ecc=ecc; Res.freq.n_cc=n_cc;
    ...
    fit_bins= idr(idth==0); fit_cc=cc(idth==0);max_cc=20;%floor(numel(cc(idth==0))/2);
    fit_out_x = fit(fit_bins(1:max_cc),fit_cc(1:max_cc),ft,fo);
    Res.freq.fit_out_x=fit_out_x;

    fit_bins= idr(idth==pi/2); fit_cc=cc(idth==pi/2);max_cc=30;%floor(numel(cc(idth==pi/2))/2);
    fit_out_y = fit(fit_bins(1:max_cc),fit_cc(1:max_cc),ft,fo);
    Res.freq.fit_out_y=fit_out_y;
    ...
    xlabel('pixel [$\mu$m]','Interpreter','latex');
    ylabel('frequency correlation','Interpreter','latex');
    title('Frequency correlation function total','Interpreter','latex');
     x0=0;y0=0;width=1000;height=1000;
    set(gcf,'position',[x0,y0,width,height])
    set(gca,'FontSize',15);
    xlim([0,bin_res*40]);ylim([-1,1]);
    saveas(gcf,'frequency_corr.png');
    close all
