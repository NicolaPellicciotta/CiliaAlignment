...
%     else
    if allfov{jj}==1; ind2=1:numel(x);
        rowsub= 1:3:size(x,1);
        colsub= 1:3:size(x,2);
        [X,Y]= meshgrid(rowsub,colsub);
        ind= sub2ind(size(x),X(:),Y(:));
    else
 
    cd(path_dir);cd(BFdir{jj});
    d_BF=dir(strcat('*X',num2str(posx),'*Y',num2str(posy),'*.movie') );
    mo=moviereader(d_BF.name);
    f_lim=[5,25];prob_cilia=0.8;box_size=4;area_min=10;
    ...
    figure(5);
%    imshow(ss);hold on;
if allfov{jj}~=1;    imagesc(IF); colormap(gray);colorbar();end
    hold on; axis image
...
Res.xx=xx;Res.yy=yy;Res.uu1=uu1;Res.vv1=vv1; 
if allfov{jj}~=1; Res.FF=FF;end
