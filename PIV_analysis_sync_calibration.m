...
%% load files
insert=insert{1};
%data_dir = strcat('/run/user/10704/gvfs/smb-share:server=sf3.bss.phy.private.cam.ac.uk,share=space/np451/syn/3.7.18/');
data_dir='/home/np451/Desktop/ependymal/sync/4.7.18/'
%a_folder=strcat('/home/np451/Desktop/ependymal/June2018/18.6.18/PIV/',insert);
a_folder=strcat('/home/np451/Desktop/ependymal/sync/4.7.18/PIV/');
mkdir(a_folder); 
...
%        mkdir(strcat(a_folder,'/',exp_name));
        ...
        frame_stack=mo.read();
        frame_stack= uint8((frame_stack)/2^(8));  %%%% converting images from 16 to 8 bit
...
%        fs=frame_stack(:,:,1:1000);
%        [X,Y,U,V] = PIV_GetData(fs,s,p);
